    ...
    try {
        ...
        if (winnerTotal >= 800) {
            ...
            if(graceSavedPlayers.length > 0) {
                io.to(roomId).emit('notification', `✨ Gracia Divina protegió a: ${graceSavedPlayers.join(", ")}. ¡No entregan puntos!`);
            }
            ...
            const animationDelay = 12000; // AJUSTE 5: 12 segundos para ver el ranking
...
io.on('connection', (socket) => {
        ...
        if (foundRoomId && foundPlayer) {
            ...
            if (foundPlayer.hasLeft) {
                ...
                if (room.gameState === 'waiting') {
                    ...
                    if (idx !== -1) { room.players.splice(idx, 1); room.players.push(foundPlayer); }
                } else { foundPlayer.isSpectator = true; }
        ...
        if (existing) { 
            ...
            if (existing.hasLeft) {
                ...
                if (room.gameState === 'waiting') {
                    ...
                    if (idx !== -1) { room.players.splice(idx, 1); room.players.push(existing); }
                } else { existing.isSpectator = true; }
    ...
    socket.on('confirmLeave', safe((choice) => {
        ...
        if (choice === 'leave_host_end' && player.isAdmin) {
            if (room.gameState === 'waiting') { io.to(roomId).emit('roomCancelled'); } 
            else { io.to(roomId).emit('notification', `⚠️ El anfitrión finalizó la partida para todos.`); io.to(roomId).emit('gameOver', { winner: 'Partida Cancelada', totalScore: 0, reason: 'cancelled' }); }
            ...
            setTimeout(() => { delete rooms[roomId]; }, 3000); return;
        }
        player.isDead = true; player.isSpectator = true; player.hasLeft = true;
    ...
    socket.on('playMultiCards', safe((cardIds) => {
        ...
        if (isAll15) {
            ...
            room.activeColor = playCards[playCards.length - 1].color; // AJUSTE 3
            ...
            if(player.hand.length === 0) { room.gameState = 'animating_win'; updateAll(roomId); setTimeout(() => calculateAndFinishRound(roomId, player), 1000); } 
        ...
        if (playCards.length === 2) {
            ...
            if (topIdx !== -1 && top.color === firstColor) {
                const min = sortedIndices; const max = sortedIndices;
             ...
             if (colorMatch || valueMatch) { isValidPlay = true; playCards.sort((a,b) => ladderOrder.indexOf(a.value) - ladderOrder.indexOf(b.value)); } 
    ...
    socket.on('playCard', safe((cardId, chosenColor, reviveTargetId, libreContext) => {
        ...
        if (card.value === 'LIBRE' && !isLibreDiscard) { 
            if (player.hand.length < 3) { socket.emit('notification', '🚫 Necesitas al menos 3 cartas.'); return; }
            ...
            io.to(roomId).emit('notification', `🕊️ ${player.name} arrojó LIBRE ALBEDRÍO...`);
        ...
        if (delay === 1000) room.gameState = 'animating_penalty';
        updateAll(roomId);
    ...
    socket.on('ripDecision', safe((d) => {
        ...
        else if (d === 'divine_save') { // AJUSTE 2: Gracia Divina vs RIP
